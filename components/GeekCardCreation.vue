  ...
  <Card title="Create Geek ID" v-if="showCreateGeekId === true">
    <div class="flex row w-full">
      ...
      <div class=" w-1/2 ml-3 p-3 ">
        <ul>
          <li class="border-b-1 border-solid p-2"><input id="input-name" class="outline outline-transparent"
              placeholder="Name"></li>
          <li class="border-b-1 border-solid p-2"><input id="input-age" class="outline outline-transparent"
              placeholder="Age"></li>
          ...
          <li class="p-2">
            ...
            <div v-if="showInterests" class="flex flex-col justify-start">
              <div class="bg-[#D9D9D9] border rounded-lg w-full mt-2">
                <input v-model="search_interest" class="ml-2 mr-1 outline outline-transparent interests-input"
                  placeholder="Search for your interests">
              </div>
              <div v-if="filteredInterests.length > 0">
                <div v-for="(interest, index) in filteredInterests.splice(0, 10)" :key="index"
                  class="text-xs m-2 bg-sky-400 text-black px-3 py-1 rounded-full font-mono inline-block">
                  <span>{{ interest }}</span>
              ...
              <div v-if="interestsToAdd.length > 0">
                <div v-for="(interest, index) in interestsToAdd" :key="index"
                  class="text-xs text-white m-2 bg-sky-800 text-black px-3 py-1 rounded-full font-mono inline-block">
                  <span>{{ interest }}</span>
    ...
    <div class="w-full relative">
      <button class="absolute bg-green-400 bottom-0 right-0 px-4 py-1  top-5 font-mono justify-center "
        v-on:click="handleGeekInfo">Next</button>
    </div>
  ...
  <Card title="Your Suggested Guilds" v-else>
    <div class="bg-linear-[#78B6F5,#ECECEC] rounded-2xl mt-15 h-100 p-2 border-3">
      ...
      <div class="flex flex-wrap gap-4 justify-center">
        <div v-for="guild in guilds.slice(0, 2)" :key="guild.id" :title="guild.title"
      ...
      <button @click="emit('close')"
        class="justify-center absolute bottom-35 right-9 flex bg-linear-[#E94FC5,#FFB7EE] w-40 h-10 rounded-2xl border-2 font-mono items-center">Done</button>
    </div>
  </Card>
