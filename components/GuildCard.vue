...
const {
  data: { value },
} = await useFetch(`/api/guild/${props.guildId}`, { method: 'get' });
guild.value = value;
console.log(guild)
...
<template>
  <Card v-if="guild" :title="guild.title" :headerActions="actions" @close="emit('close')">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
      ...
      <div>
        ...
        <div class="space-y-4 p-4 bg-linear-[180deg,#78B6F5,#ECECEC] rounded-b-lg border-3 border-t-0 overflow-auto">
          <div v-for="event in guild.GuildEvent" :key="event.id" class="bg-blue-500 rounded-lg border-3">
            <div class="relative">
              ...
              <button
                class="absolute bottom-2 right-2 bg-[#FFB235] hover:cursor-pointer hover:bg-orange-500 transition-colors px-4 py-1 rounded font-mono border-3 rounded-lg">
                REGISTER
              </button>
            </div>
      ...
      <div>
        ...
        <div class="bg-[#F2F2F2] rounded-b-lg border-3 border-t-0">
          <div v-for="member in guild.GuildMembers" :key="member.User.username"
